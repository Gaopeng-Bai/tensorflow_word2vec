...
class word2vec():
    ...
    def __init__(self,
                 ...
                 logdir='tmp/word2vec',  # 模型保存目录
                 model_path="model"  # 模型路径，用于训练好后加载模型用
                 ):
    ...
    def build_graph(self):
        ...
        with self.graph.as_default():  # 定义graph图，在“graph”中定义运算和张量。
            ...
            self.embed_vector = tf.placeholder(tf.int32, shape=[self.vocab_size, 1])  # 中心词
            ...
            # generate embedd vector and similarity between words by words id
            ...
            # generate words id by embedd vector
            tA_A = tf.matmul(tf.transpose(self.embedding_dict), self.embedding_dict)
            tA_A_inv = tf.matrix_inverse(tA_A)
            product = tf.matmul(tA_A_inv, tf.transpose(self.embedding_dict))
            self.solution = tf.matmul(self.embed_vector, product)
            # matmul：矩阵相乘
            # 变量初始化
   ...
   # 5、计算相似概率，测试词的id，只选取相似概率前10个
    def cal_similarity(self, test_word_id_list, top_k=10):
        # 相似度矩阵
        sim_matrix, embed,solution = self.sess.run([self.similarity, self.test_embed,self.solution], feed_dict={self.test_word_id: test_word_id_list})
        ...
        return test_words, near_words, sim_mean, sim_var, embed,solution  # 返回测试词，相似词，相似度均值，相似度方差
...
if __name__ == '__main__':
    ...
    if os.path.exists("tmp/test.txt") and os.listdir("model"):
        # 创建模型，训练
        with open("tmp/test.txt", "rb") as fp:  # Pickling
            unpickler = pkl.Unpickler(fp)
            try:
        ...
        with open("tmp/test.txt", "wb") as fp:  # Pickling
            pkl.dump(word_list, fp)
        # 创建模型，训练
        w2v = word2vec(vocab_list=word_list,  # 词典集
                       embedding_size=200,  # 词向量的维度
                       win_len=2,  # 窗口大小
        ...
        for i in range(num_steps):
            for j in range(len(sentence_list)):
                ...
                sent = sentence_list[j]
                # 如果len(sentence_list) <  num_steps,会有重复训练的行，如num_steps=20，len(sentence_list)=10
                # 0 % 10 = 0，1 % 10 = 1，...,10 % 10 = 0，11 % 10 = 1，12 % 10 = 2...
                # 这样有些就被训练多次了
                w2v.train_by_sentence([sent], epoch=i)  # 按行训练模型，我们认为一行代表一个样本

        w2v.save_model()  # 保存模型，加训练好的模型参数保存

    # 测试
    test_word = ["萧炎"]
    test_id = [word_list.index(x) for x in test_word]  # 测试单词在词典word_list中的索引，即为单词id
    test_words, near_words, sim_mean, sim_var, embed, wordid = w2v.cal_similarity(test_id)  # 计算相似度
    # print("Test_words:{}, near_words: {}, sim_mean:{}, sim_var: {}, embed: {}"
    #       .format(test_words, near_words, sim_mean, sim_var,embed))
    print(wordid)